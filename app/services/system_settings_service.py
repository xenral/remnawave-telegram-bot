...
class BotConfigurationService:
    ...
    CATEGORY_KEY_OVERRIDES: dict[str, str] = {
        ...
        'MULTI_CURRENCY_ENABLED': 'PAYMENT',
        'DEFAULT_BALANCE_CURRENCY': 'PAYMENT',
        'DEFAULT_DISPLAY_CURRENCY': 'PAYMENT',
        'DEFAULT_REPORTING_CURRENCY': 'PAYMENT',
        'SUPPORTED_BALANCE_CURRENCIES': 'PAYMENT',
        'SUPPORTED_DISPLAY_CURRENCIES': 'PAYMENT',
    ...
    CHOICES: dict[str, list[ChoiceOption]] = {
        ...
        'DEFAULT_BALANCE_CURRENCY': [
            ...
            ChoiceOption('IRR', '﷼ IRR'),
        ],
        'DEFAULT_DISPLAY_CURRENCY': [
            ChoiceOption('', '↔️ Как валюта баланса'),
            ChoiceOption('RUB', '₽ RUB'),
            ChoiceOption('IRR', '﷼ IRR'),
            ChoiceOption('TMN', 'TMN (Toman, display-only)'),
        ],
        'DEFAULT_REPORTING_CURRENCY': [
            ChoiceOption('RUB', '₽ RUB'),
            ChoiceOption('IRR', '﷼ IRR'),
    ...
    SETTING_HINTS: dict[str, dict[str, str]] = {
        ...
        'MULTI_CURRENCY_ENABLED': {
            'description': (
                'Включает валютно-ориентированную работу цен и пополнений. '
                'При включении цены периодов/трафика редактируются в разрезе выбранной валюты.'
            ),
            'format': 'Булево значение.',
            'example': 'true',
            'warning': (
                'Перед включением настройте цены по валютам и FX-курсы. '
                'Иначе часть платежей может стать недоступной.'
            ),
            'dependencies': 'DEFAULT_BALANCE_CURRENCY, SUPPORTED_BALANCE_CURRENCIES, Currency Rates',
        },
        'DEFAULT_BALANCE_CURRENCY': {
            'description': 'Валюта баланса новых пользователей и fallback для расчётов.',
            'format': 'Выберите валюту из списка.',
            'example': 'RUB',
            'warning': 'TMN не должен использоваться как валюта баланса (только display-валюта над IRR).',
            'dependencies': 'SUPPORTED_BALANCE_CURRENCIES',
        },
        'DEFAULT_DISPLAY_CURRENCY': {
            'description': (
                'Валюта отображения по умолчанию. '
                'TMN используется как display-алиас для IRR (1 TMN = 10 IRR).'
            ),
            'format': 'Выберите валюту или «Как валюта баланса».',
            'example': 'TMN',
            'warning': 'Для TMN рекомендуется баланс в IRR.',
            'dependencies': 'SUPPORTED_DISPLAY_CURRENCIES',
        },
        'DEFAULT_REPORTING_CURRENCY': {
            'description': 'Единая валюта для агрегированных отчетов и статистики.',
            'format': 'Выберите валюту из списка.',
            'example': 'RUB',
            'warning': 'Смена валюты отчётности требует корректных FX-курсов.',
            'dependencies': 'currency_rates',
        },
        'SUPPORTED_BALANCE_CURRENCIES': {
            'description': 'Список валют баланса через запятую.',
            'format': 'Коды ISO через запятую, например RUB,IRR.',
            'example': 'RUB,IRR',
            'warning': 'Не добавляйте TMN сюда: TMN только display-валюта.',
        },
        'SUPPORTED_DISPLAY_CURRENCIES': {
            'description': 'Список поддерживаемых валют отображения через запятую.',
            'format': 'Коды через запятую.',
            'example': 'RUB,IRR,TMN',
            'warning': 'TMN допустим только как display-формат поверх IRR.',
        },
